...
export const OrganizationTabList = ({ showActiveStatus = true, currentPage = '', scrollToItem }: { showActiveStatus?: boolean, currentPage?: string, scrollToItem?: (key: string) => void }) => {
  ...
  useEffect(() => {
    // 监听标签关闭事件
    uiEventBus.on('CLOSE_TAB', (orgId: string, closedIds: string[], activeTabId: string) => {
      //如果关闭的标签是当前标签，则滚动到下一个标签
      if (activeTabId) {
        scrollToItem?.(activeTabId);
      }
    });
  }, []);
