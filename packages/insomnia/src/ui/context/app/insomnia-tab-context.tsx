...
export const InsomniaTabProvider: FC<PropsWithChildren> = ({ children }) => {
  ...
  const closeTabById = useCallback((id: string) => {
    ...
    //计算关闭后要激活的标签ID
    const activeTabId = currentTabs.activeTabId === id ? newTabList[Math.max(index - 1, 0)]?.id : currentTabs.activeTabId as string;
    updateInsomniaTabs({
      organizationId,
      tabList: newTabList,
      activeTabId,
    });
    uiEventBus.emit('CLOSE_TAB', organizationId, [id], activeTabId);
